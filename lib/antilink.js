...
handler.before = async function (m, { isAdmin, isBotAdmin, Phoenix }) {
//  if (isAdmin) { m.reply('*The group link should not be sent, admin :v*') }
  ...
  let handler = function (m) {
    ...
    Phoenix.sendMessage(m.chat, { delete: { remoteJid: m.chat,  id: m.quoted.id, participant: m.quoted.sender } })
  } 
  ...
  if (chat.antiLink && isGroupLink && !isAdmin && !m.isBaileys && m.isGroup) {
    ...
    if (isBotAdmin) {
      ...
      await Phoenix.groupParticipantsUpdate(m.chat, [m.sender], "remove")
      await Phoenix.sendMessage(m.chat, { delete: m.key })
    }
  }
  return true
}
